...
def find_CROSS_MAS(g):
    a = anim.AnimGrid(anim.Screen(1260,1260, 'Guard Walk'), g.dim_x, g.dim_y, use_chars=True)
    a.color_map = { 'X': 'DARK_GRAY', 'M': 'BROWN', 'A': 'RED', 'S': 'BROWN', 'default': 'DARK_GRAY' }
    ...
    g_solved = Grid(g.dim_y, g.dim_x, val=' ')
    for pos in a_positions :
        g_solved.set(pos, 'A')
        n_pos = g.neighbour_pos(pos)
        for key in ('NE', 'SW', 'NW', 'SE') :
            g_solved.set(n_pos[key], g.get(n_pos[key]))
    a.draw(g_solved, 6000)
