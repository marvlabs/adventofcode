...
#### Animation
dim = 141
a = anim.AnimGrid(anim.Screen(dim*8, dim*8+50, 'Run Camel Run'), dim, dim,
    use_chars=False, status_area=50, 
    title='AOC 2024-16 Run Camel Run', movie = "AOC_2024-16_Camel_Race")
a.color_map = { '.': 'BLACK', '#': 'DARK_BLUE', 'O': 'ORANGE', ' ': 'DARK_GRAY', 'default': 'PINK' }
####
...
def dijkstra_maze(maze, start_pos, start_dir, end_pos) :
    ...
    count=0
    a.draw(maze, 1000, f'Junctions: {count:6d}')
    ...
    while len(unvisited) :
        ...
        if count % (1 if count < 50 else (5 if count < 800 else (1 if count < 860 else (25 if count > 3500 else 15)))) == 0 : 
            for pos in path_from_node(end_node if end_node else node) : maze.set(pos, 'O')
            a.draw(maze, 30, f'Junctions: {count:6d}')
        for pos in node['edge'] : maze.set(pos, ' ')
    ...
    a.draw(maze, 3000, f'Junctions: {count:6d}')
    a.save_movie()
